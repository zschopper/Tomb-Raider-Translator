...
namespace TRTR
{
    ...
    {
        internal static bool ProcessStream(string name, Stream inStream, long contentLength, Stream outStream, TranslationProvider tp)
        {
            ...
            if (lines.Length < 4)
                throw new Exception(string.Format("Hibás sch fájl: \"{0}\" nincs adat.", name)); // trans
            if (lines[0] != "Version 1")
                throw new Exception(string.Format("Hibás sch fájl: \"{0}\" ismeretlen verzió.", name));// trans
            if (lines[1] != "TextEntry")
                throw new Exception(string.Format("Hibás sch fájl: \"{0}\" hibás fájlformátum. Sor: 2", name));// trans
            if (lines[2] != "line0")
                throw new Exception(string.Format("Hibás sch fájl: \"{0}\" hibás fájlformátum. Sor: 3", name));// trans
            #endregion
                        ...
                        if (m2.Success)
                        {
                            ...
                            if (tp.UseContext)
                                context = new Dictionary<string, string> { 
                                    ...
                                    {"filename", name}, 
                                    {"hash", name},
                                    {"bigfile", "external_file"},
                                    {"time", timeStr},
                                };
                            //context = new Dictionary<string> { 
                            //        timeStr,
                            //    };
                            string translatedText = TRGameInfo.Conv.ToGameFormat(tp.GetTranslation(text, null, context));
                            if (write)
